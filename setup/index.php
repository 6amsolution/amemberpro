    ...
    function e($varName, $default = null)
{
    ...
    // Check if $value is not null before applying htmlentities
    if ($value !== null) {
        return htmlentities($value);
    }

    return null; // or you can choose to return an empty string or any default value
}
...
CUT;
    ...
    public function run()
    {
        ...
        if ($step != 5 && !$this->check_for_existence()) {
            $this->title = "aMember Setup: Already installed";
            return $this->display();
        }

        if (!$this->check_for_writeable()) {
            $this->title = "aMember Setup: Folder permissions must be fixed";
            return $this->display();
        }
    ...
    public function check_for_existence()
    {
        // Implement the logic to check for existence
        // For example, check if the setup is already installed
        // You might need to replace this with your specific logic
